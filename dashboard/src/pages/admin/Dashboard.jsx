    ...
    return (
        <div className="space-y-10 animate-fade-in relative z-10">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div className="space-y-1.5 font-sans">
                    <h1 className="text-4xl md:text-5xl font-serif font-black text-foreground tracking-tight leading-[1.1]">
                        Institutional <span className="text-primary italic">Oversight</span>
                    </h1>
                    <p className="text-muted-foreground text-lg max-w-2xl font-medium tracking-tight opacity-70">
                        Strategic management of school-wide infrastructure and resource allocation.
                    </p>
                    ...
                    {scheduledToday.length > 0 && (
                        <div className="mt-4 flex items-center gap-3 px-4 py-2 bg-primary/5 border border-primary/10 rounded-full w-fit group">
                            ...
                            <span className="text-[10px] font-black text-primary uppercase tracking-[0.2em] group-hover:tracking-tight transition-all duration-500">
                                {scheduledToday.length} Deployments active today
                            </span>
                        </div>
                    )}
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatsCard title="Awaiting" value={pendingApproval.length} subtitle="Decision required" icon={Clock} color="amber" />
                <StatsCard title="In Flight" value={inProgress.length} subtitle="Operational task" icon={Wrench} color="blue" />
                <StatsCard title="Critical" value={escalated.length} subtitle="High priority" icon={AlertTriangle} color="red" />
                <StatsCard title="Success" value={completed.length} subtitle="Total restorations" icon={CheckCircle} color="green" />
            </div>
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 bg-white rounded-2xl border border-border p-8 shadow-sm">
                    <div className="flex items-center justify-between mb-8 px-2">
                        <div>
                            ...
                            <p className="text-xs text-muted-foreground mt-1 font-medium italic opacity-60">Authorize and prioritize campus work orders</p>
                        </div>
                    ...
                    <div className="space-y-1">
                        {pendingApproval.length === 0 ? (
                            <div className="text-center py-20 rounded-xl bg-slate-50 border border-dashed border-border flex flex-col items-center justify-center opacity-60">
                                ...
                                <h3 className="text-lg font-bold text-foreground">Queue Synchronized</h3>
                                <p className="text-xs text-muted-foreground max-w-[200px] mt-2 font-medium">No pending requests require immediate authorization.</p>
                            </div>
                        ) : (
                                        ...
                                        <div className="group flex items-center gap-5 p-4 rounded-xl hover:bg-slate-50 transition-all border border-transparent hover:border-border">
                                            <div className="w-10 h-10 rounded-full bg-secondary flex items-center justify-center flex-shrink-0 group-hover:bg-white shadow-sm transition-colors">
                                                <div className={cn(
                                                    ...
                                                    req.priority === 'Critical' ? 'bg-red-500' : 'bg-primary'
                                                )} />
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <p className="text-[10px] font-bold text-muted-foreground mt-0.5 opacity-60">{req.school_name} · {req.reported_by_name}</p>
                                            </div>
                                            <div className="flex flex-col items-end gap-1 flex-shrink-0">
                                                <StatusBadge status={req.priority} size="sm" />
                                                <span className="text-[9px] font-black text-muted-foreground/40 uppercase tracking-widest leading-none mt-1">{req.created_at ? format(req.created_at.toDate(), 'MMM dd') : ''}</span>
                                            </div>
                                        </div>
                                    </Link>
                                </motion.div>
                ...
                <div className="bg-white rounded-2xl border border-border p-8 shadow-sm">
                    <h2 className="text-xl font-serif font-bold text-foreground mb-8">System Velocity</h2>
                    <div className="space-y-8">
                        {[{ label: 'Pending Authorization', count: pendingApproval.length, color: 'bg-amber-500' }, { label: 'Assigned Operations', count: inProgress.length, color: 'bg-primary' }, { label: 'Restored Assets', count: completed.length, color: 'bg-emerald-500' }, { label: 'Authority Attention', count: escalated.length, color: 'bg-rose-500' }].map(({ label, count, color }) => (
                            <div key={label} className="group">
                                <div className="flex justify-between items-end mb-2">
                                    ...
                                    <div className="flex items-baseline gap-1">
                                        ...
                                        <span className="text-[10px] text-muted-foreground opacity-40">/ {requests.length}</span>
                                    </div>
                                </div>
                    ...
                    <div className="mt-12 p-6 rounded-2xl bg-sidebar rounded-2xl text-white shadow-xl shadow-primary/10">
                        <div className="flex items-center gap-3 mb-3">
                            <TrendingUp className="w-4 h-4 text-primary" />
                            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white/30">Strategic Insight</span>
                        </div>
                        <p className="text-xs font-medium text-white/70 leading-relaxed italic">
                            The campus restoration pipeline is <span className="text-primary font-bold not-italic">Synchronized</span>. Resource allocation is currently meeting institutional benchmarks.
                        </p>
                    </div>
                </div>
            </div>
